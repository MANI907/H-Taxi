...
<v-card style="width:300px; margin-left:5%;" outlined>
    ...
    <v-card-title v-if="value._links">
        AllocationManagement # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
    </v-card-title >
    <v-card-title v-else>
  ...
  export default {
    ...
    methods: {
      ...
      async ConfirmAllocation(){
        try{
          var temp = await axios.put(axios.fixUrl(this.value._links.confirmAllocation.href))
          this.value = temp.data;
          this.editMode = false;
        }catch(e){
          alert(e.message)
