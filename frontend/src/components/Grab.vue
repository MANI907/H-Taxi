...
<v-card style="width:300px; margin-left:5%;" outlined>
    ...
    <v-card-title v-if="value._links">
        Grab # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
    </v-card-title >
    <v-card-title v-else>
        Grab
    </v-card-title >
    ...
    methods: {
      ...
      async save(){
        try{
          ...
          if(this.isNew){
            temp = await axios.post(axios.fixUrl('/grabs'), this.value)
          }else{
      ...
      async ChangeGrabStatus(){
        try{
          var temp = await axios.put(axios.fixUrl(this.value._links.changeGrabStatus.href))
          this.value = temp.data;
          this.editMode = false;
        }catch(e){
          alert(e.message)
